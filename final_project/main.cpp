...
int numMaterials = 8;
enum Material {HEAD, FRAME, CYMBAL, SHINY_BLUE, FLAT_GREY, GOLD, TEAL, UNKNOWN};
   ...
   */
   ...
   if (!strcmp(id, "RIFF")) { //we had 'RIFF' let's continue
      ...
      if (!strcmp(id,"WAVE")) { //this is probably a wave file since it contained "WAVE" 
         ...
         if (format_length != bits_per_sample) {
            printf("CHECK DIS OUT\n");
         }
          
         fread(id, sizeof(char), 4, fp); //read in 'data'
         //printf("'data': %d %d %d %d\n", id[0], id[1], id[2], id[3]);
         
         fread(&data_size, sizeof(short), 1, fp); //how many bytes of sound data we have
         //printf("data_size: %u\n", data_size);
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
   if (action == GLFW_PRESS) {
      ...
      if (key == GLFW_KEY_SPACE) {
         bdHeadQuiverOn = 1;
         //PlaySound("audio/bassdrum.wav", NULL, SND_ASYNC);
         playSound("audio/bass_drum.wav");
      }
      
      if (key == GLFW_KEY_J) {
         sdHeadQuiverOn = 1;
         playSound("audio/snare_drum.wav");
      }
