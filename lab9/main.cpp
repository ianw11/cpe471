...
glm::vec3 leaderStart(-5, 0, 0);
float leaderDirection = 0;
float leaderTime = 0.0;
float leaderSpeed = .05;
int numFollowers = 4;
glm::vec3 followers[5] = {
   vec3(-6.0, 0,  0.0),
   vec3(-4.0, 0, -1.0),
   vec3( 0.0, 0, -2.0),
   vec3(-5.0, 0,  4.0),
...
/* Lab 9 helper functions */
glm::vec3 getLeaderPosition() {
   glm::vec3 res = vec3(leaderStart);
   
   res.x += 2 * sin(leaderTime);
   res.z += 7 * cos(leaderTime);
   
   /*
   if (leaderDirection < 1) {
      res.z += leaderTime;
   ...
   */
   ...
   leaderTime += leaderSpeed;
   return res;
}


void doLab9(int nIndices) {
	
	glm::vec3 leaderPos = getLeaderPosition();
	
	g_trans = leaderPos;
	SetModel(leaderDirection);
	glDrawElements(GL_TRIANGLES, nIndices, GL_UNSIGNED_INT, 0);
	
	
	for (int followerNdx = 0; followerNdx < numFollowers; ++followerNdx) {
	   glm::vec3 oldPos = followers[followerNdx];
	   
	   /* Get direction to leader */
	   glm::vec3 diff = glm::normalize(leaderPos - oldPos);
	   /* Head toward leader with some speed */
	   alpha[followerNdx] = diff * GLfloat(.1);
	   
	   for (int b = followerNdx + 1; b < numFollowers; ++b) {
	      glm::vec3 collDiff = GLfloat(1.0) / (oldPos - followers[b]);
	      collDiff.y = 0;
	      
	      beta[followerNdx] += collDiff;
	      beta[b] += collDiff * GLfloat(-1);
	   }
	   
	   glm::vec3 leaderDiff = GLfloat(1.0) / (oldPos - leaderPos);
	   leaderDiff.y = 0;
	   if (isinf(leaderDiff.x))
	      leaderDiff.x = 0;
	   if (isinf(leaderDiff.z))
	      leaderDiff.z = 0;
	      
	   beta[followerNdx] += leaderDiff;
	      
	   
	   followers[followerNdx] = oldPos + alpha[followerNdx] + glm::normalize(beta[followerNdx]) * GLfloat(.03);
	   beta[followerNdx] = glm::vec3(0, 0, 0);
	}
	
	
	glm::vec3 centerOfMass(0, 0, 0);   
	centerOfMass += leaderPos;
	
	for (int c = 0; c < numFollowers; ++c) {
	   centerOfMass += followers[c];
	}
	
	centerOfMass /= GLfloat(numFollowers + 1.0);
	
	
	for (int applyFollower = 0; applyFollower < numFollowers; ++applyFollower) {
	   glm::vec3 curr = followers[applyFollower];
	   curr += glm::normalize(centerOfMass - curr) * GLfloat(.01);
	   g_trans = curr;
	   SetModel();
	   glDrawElements(GL_TRIANGLES, nIndices, GL_UNSIGNED_INT, 0);
	}
